...
def load_phi_files_latlon(dataDir, lon, sc):
    ...
    # L_s = sorted([float(f[4:(len(f) - 2)]) for f in s_files])
    p = re.compile("\d+")
    # print "N", [p.findall(f) for f in n_files]
    # print "S", [p.findall(f) for f in s_files]
    ...
    latvec = np.unique(np.array(lat_n))
    lonvec = np.unique(np.array(lon_n))

    if lon in lonvec:
        N_arr = np.zeros([sc.NUM_E, sc.NUM_STEPS, len(latvec)])
        S_arr = np.zeros([sc.NUM_E, sc.NUM_STEPS, len(latvec)])
